...
def step_implications(cfg: Config) -> None:
    ...
    tags_lookup = tags_df.select([
        pl.col("name").alias("tag_name"),
        pl.col("id").cast(pl.Int64).alias("tag_id"),
    ])

    if dag_edges:
        raw_dag = pl.DataFrame(dag_edges, schema=["a", "b"], orient="row").unique()
        dag = (
            raw_dag
            .join(tags_lookup, left_on="a", right_on="tag_name", how="inner")
            .rename({"tag_id": "a_id"})
            .join(tags_lookup, left_on="b", right_on="tag_name", how="inner")
            .rename({"tag_id": "b_id"})
            .select([pl.col("a_id").cast(pl.Int64), pl.col("b_id").cast(pl.Int64)])
        )
    else:
        dag = pl.DataFrame(schema={"a_id": pl.Int64, "b_id": pl.Int64})
    ...
    used_a = dag.get_column("a_id").to_list() if not dag.is_empty() else []
    used_b = dag.get_column("b_id").to_list() if not dag.is_empty() else []
    used: set[int] = {int(x) for x in (used_a + used_b)}

    stats = (
        _read_tags_csv(cfg.tags_csv)
        .filter(pl.col("id").is_in(list(used)))
    ...
    if pairs:
        pl.DataFrame(pairs, schema={"tag_id": pl.Int32, "ancestor_id": pl.Int32, "depth": pl.Int8}, orient="row")\
          .write_parquet(out_cache, compression="zstd")
        log(f"[impl] ancestral cache written: {len(pairs):,} pairs")
    else:
        ensure_dir(out_cache.parent)
        pl.DataFrame(schema={"tag_id": pl.Int32, "ancestor_id": pl.Int32, "depth": pl.Int8})\
          .write_parquet(out_cache, compression="zstd")
