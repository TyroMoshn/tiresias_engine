...
"""
TAG2VEC - offline tag embeddings built from co-occurrence (PMI) and pool-derived signals.
Output:
  - features/tag2vec.parquet: [tag_id:int32, vec:list[float32]]
  - features/tag2vec_knn.parquet (optional): [tag_id:int32, nn_tag_id:int32, sim:float32]
  - features/tag2vec_meta.json: auxiliary run metadata

Idea:
  1) Build an undirected tag graph with weights w_ij from:
     - tag_pmi.parquet (PMI > 0)
     - tag_co_from_pools.parquet (pool-based weights)
  2) Convert weights to shifted PMI: X_ij = max(0, log((w_ij * S) / (d_i * d_j)) - shift)
     where d_i = sum_j w_ij, S = sum_{i<j} w_ij.
  3) Construct a sparse matrix X and perform truncated SVD (randomized_svd).
     Embeddings: E = U * sqrt(Sigma). Normalize so that ||e_i|| = 1.
  4) Save the parquet and (optionally) top-K nearest neighbors by cosine similarity.

Dependencies: polars, numpy, (preferably) scipy.sparse and sklearn (randomized_svd).
If scipy/sklearn are unavailable, fall back to dense SVD for small vocabularies (<= 50k tags).
"""
...
# Enforce high-performance dependencies (no fallbacks)
if 'sp' not in globals() or sp is None:
    raise ImportError("SciPy (sparse, svds) is required for tag2vec")
if 'faiss' not in globals() or faiss is None:
    raise ImportError("faiss is required for tag2vec KNN")
...
def _load_edges_from_pmi(cfg: Config, keep: set[int]) -> Dict[Tuple[int, int], float]:
    """Reads tag_pmi.parquet and returns symmetric weights for positive PMI values."""
...
def _load_edges_from_pools(cfg: Config, keep: set[int]) -> Dict[Tuple[int, int], float]:
    """Reads tag_co_from_pools.parquet and returns symmetric weights."""
...
def _merge_edges(e1: Dict[Tuple[int, int], float], e2: Dict[Tuple[int, int], float], alpha: float) -> Dict[Tuple[int, int], float]:
    """Mix two weight sources: w = w_pmi + alpha * w_pools."""
...
def _to_ppmi(edges: Dict[Tuple[int, int], float], tag_ids: List[int], shift: float) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    """Converts weights into PPMI values and returns COO triples (rows, cols, data) for a symmetric matrix.
    Returns indices as positional (0..V-1)."""
    if not edges:
...
def _embed_sparse(rows: np.ndarray, cols: np.ndarray, data: np.ndarray, V: int, dim: int) -> Tuple[np.ndarray, np.ndarray]:
    """Performs truncated SVD on the sparse PPMI:
    1) Try scipy.sparse.linalg.svds (ARPACK) - fast and avoids 'gesdd'.
    2) If svds fails, use randomized_svd(n_iter=2) as a quick fallback.
    Returns (E, S): E.shape = (V, dim), with L2-normalized rows."""
    if rows.size == 0:
...
def _write_knn(cfg: Config, tag_ids: List[int], E: np.ndarray, k: int) -> None:
    """Writes top-K cosine neighbors.
    Prefer FAISS (IndexFlatIP), since E is already L2-normalized => cosine == dot product.
    Use batched search; build the index once."""
    if k <= 0:
        return
    V, D = E.shape
    if V == 0:
    ...
    # faiss loves C-contiguous float32
...
def step_tag2vec(cfg: Config) -> None:
    ...
    meta = {
        ...
        "shift": float(shift),
        "knn_k": int(knn_k),
        "vectors": "data/features/tag2vec.parquet",
        "knn": "data/features/tag2vec_knn.parquet",
