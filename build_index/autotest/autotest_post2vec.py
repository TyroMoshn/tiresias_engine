...
def inspect_sq8_index(idx_path: Path, expected_rows: int, expected_dim: int) -> CheckResult:
    if not idx_path.exists():
        return CheckResult("post2vec_sq8.index", "SKIP", [f"File not found: {idx_path}"])
    if faiss is None:
        return CheckResult("post2vec_sq8.index", "SKIP", ["faiss module not available"])
    ...
    # Enable multi-threading for search benchmark if supported
    try:
    ...
    for _ in range(5):
        _ = idx.search(q, 100)
    avg_ms = (time.perf_counter() - t0) / 5.0 * 1000.0

    details = [
        f"ntotal={ntotal:,}",
        f"dim={dim}",
        f"type={idx.__class__.__name__}",
        f"size={size_mb:.1f} MB (target <= 850 MB)",
        f"search_latency={avg_ms:.2f} ms (target <= 150 ms linear scan)",
    ]
    status = "PASS"
    if ntotal != expected_rows:
        details.append(f"Row count mismatch (expected {expected_rows:,})")
        status = "FAIL"
    if expected_dim > 0 and dim != expected_dim:
        details.append(f"Dim mismatch (expected {expected_dim})")
        status = "FAIL"
    if size_mb > 850:
        details.append("File size exceeds 850 MB target")
        status = "WARN"
    if avg_ms > 200:
        details.append("Search latency exceeds 200 ms target")
        status = "WARN"
    return CheckResult("post2vec_sq8.index benchmark", status, details)
...
DEFAULT_CATEGORY_WEIGHTS: Dict[int, float] = {
    1: 2.50,  # Artist
    4: 2.00,  # Character
    3: 1.80,  # Copyright
    5: 1.30,  # Species
...
def load_tag_metadata(root: Path, tag_ids: Iterable[int]) -> Tuple[Dict[int, float], Dict[int, int]]:
    ids = list(set(int(t) for t in tag_ids))
    if not ids:
        return {}, {}
    ...
    if tags_parquet.exists():
        ...
        df = lf.select(
            pl.col("tag_id").cast(pl.Int64).alias("tag_id"),
        ...
        df = lf.select(
            ...
            pl.lit(1.0).alias("idf"),
            pl.coalesce([pl.col("category"), pl.lit(0)]).cast(pl.Int32).alias("category"),
        ).collect(engine="streaming")
    idf = {int(rec["tag_id"]): float(rec["idf"]) for rec in df.iter_rows(named=True)}
    cats = {int(rec["tag_id"]): int(rec["category"]) for rec in df.iter_rows(named=True)}
    return idf, cats
    ...
    for pid, tags in post_tags.items():
        ...
        for tag_id in tags:
            ...
            if tv is None or tv.size == 0:
                continue
            cat = cat_map.get(tag_id, 0)
            c_w = float(cat_weights.get(cat, 1.0))
            if c_w <= 0.0:
                continue
            w = float(idf.get(tag_id, 1.0)) * c_w
            if w != 0.0:
                vec += w * tv
        norm = float(np.linalg.norm(vec))
        if norm > 1e-6:
...
def verify_samples(
    ...
    idf, cat_map = load_tag_metadata(root, sample_tag_ids)
    expected_vectors = compute_expected_vectors(
        post_tags, tag_vecs, idf, cat_map, DEFAULT_CATEGORY_WEIGHTS, dim
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    ap.add_argument("--root-data", type=Path, default=default_root, help="Path to data directory.")
    ...
    sq8_path = features / "post2vec_sq8.index"
    ...
    # Check SQ8 index
    if sq8_path.exists():
        sq8_res = inspect_sq8_index(sq8_path, expected_rows, expected_dim)
        suite.add(sq8_res)
        if index_obj is None and faiss is not None:
            try:
                index_obj = faiss.read_index(sq8_path.as_posix())
            except Exception:
                pass
    elif not idx_path.exists():
        suite.add(CheckResult("FAISS index presence", "FAIL", ["Neither post2vec_faiss.index nor post2vec_sq8.index found."]))
