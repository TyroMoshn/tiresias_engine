# autotest_index.py
# Location: build_index/autotest/autotest_index.py
    ...
    def paths(self) -> Dict[str, Path]:
        ...
        if Config is not None:
            cfg = Config(
                ...
                uploaders_csv=self.root / "uploaders_uploads.csv",
            )
            return {
                ...
                "uploaders_csv": cfg.uploaders_csv,
                "csv_db": cfg.csv,
...
def check_tag_bitmaps_against_post_tags(env: Env, sample_tags: int = 5) -> CheckResult:
    ...
    if roar_files:
        ...
        problems: list[str] = []
        details: list[str] = []
        post_ids_bin = env.paths["mmaps"] / "post_ids.bin"
        post_ids_arr = np.memmap(post_ids_bin, mode="r", dtype=np.int64) if post_ids_bin.exists() else None
        ...
        for rf in sample:
            ...
            ids = set(lf.collect(engine="streaming").get_column("post_id").to_list())
            bm_ids = list(bm)
            if post_ids_arr is not None and bm_ids and max(bm_ids) < len(post_ids_arr):
                roar_ids = set(post_ids_arr[bm_ids].tolist())
            else:
                roar_ids = set(map(int, bm_ids))
    ...
    if not env.quick:
        ...
        if recs:
            ...
            if post_ids_bin.exists():
                post_ids_arr = np.memmap(post_ids_bin, mode="r", dtype=np.int64)
                if bm_ids and max(bm_ids) < len(post_ids_arr):
                    roar_ids = set(post_ids_arr[bm_ids].tolist())
                else:
                    roar_ids = set(map(int, bm_ids))
            else:
        ...
        for i in range(n):
            ...
            if not env.quick and len(ids) > 0:
                ...
                if i == 0:
                    ...
                    if not top_set.issubset(set(ids)):
...
def _compute_top_set(env: Env, tag_id: int, K: int) -> set[int]:
    ...
    return set(tbl.collect(engine="streaming").get_column("post_id").to_list())
...
def check_against_csv(env: Env, limit_rows: int = 0) -> CheckResult:
    ...
    if not db_csv.exists():
        return CheckResult("CSV vs Parquet", "SKIP", [f"posts.csv not found at {posts_csv}"])
...
def perf_profile(env: Env) -> CheckResult:
    ...
    if pt.exists():
        t = time.perf_counter()
        cnt = pl.scan_parquet(f"{pt.as_posix()}/**/*.parquet").select(pl.len()).collect(engine="streaming")[0,0]
...
REM Portable root resolution via %~dp0
set TESTS_DIR=%~dp0
...
def main() -> int:
    default_data_root = Path(os.environ.get("TIRESIAS_DATA_ROOT", HERE.parent.parent / "data")).resolve()
    ap = argparse.ArgumentParser(description="TIRESIAS_ENGINE – Autotest for Indexed Data")
    ap.add_argument("--root", type=Path, default=default_data_root, help="Path to data root")
