...
def load_tag_vectors(features_dir: Path) -> Tuple[pl.DataFrame, Optional[pl.DataFrame], dict]:
    ...
    if not vec.exists():
        raise FileNotFoundError(f"Not found {vec}")
...
def load_tags_table(root_data: Path) -> pl.DataFrame:
    """
    Artifacts, NOT csv: we prefer tags.parquet (read from stats_stage),
    otherwise tags_dict.parquet (read from tags_stage). We expect 'tag_id' and 'tag' in both.
    """
    ...
    raise FileNotFoundError("Not found: tags.parquet or tags_dict.parquet")

def load_alias_map_if_any(root_data: Path) -> Dict[str, str]:
    """
    Quickly read CSV aliases, do not recalculate.
    Return map: alias(lower) -> canon(lower), only status 'active'.
    """
...
def vecs_from_polars(df: pl.DataFrame) -> Tuple[np.ndarray, List[int]]:
    if "vec" not in df.columns or "tag_id" not in df.columns:
        raise ValueError("Expecting columns  'tag_id' and 'vec' in tag2vec.parquet")
...
# --------------------- CLI ---------------------
def main():
    ap = argparse.ArgumentParser(description="The tag2vec test shows the nearest neighbors for a tag (by id or name).")
    ap.add_argument("--root-data", type=Path, default=Path(r"I:\TIRESIAS_ENGINE\data"))
    ap.add_argument("--tag", type=str, required=True, help="Tag name (string) or its ID (number)")
    ap.add_argument("--topk", type=int, default=10)
    ap.add_argument("--recompute", action="store_true", help="Before displaying, recalculate tag2vec (only this step)")
    ap.add_argument("--suggest", action="store_true", help="If name not found, show substring suggestions")
    # recompute params (if --recompute)
    ...
    if args.recompute:
        ...
        if rc != 0:
            print("[recompute] ended with the code", rc)
            return

    # (1) artefacts
    vec_df, knn_df, meta = load_tag_vectors(features)
    print("Vectors loaded:", vec_df.height)
    if knn_df is not None:
        print("KNN rows:", knn_df.height)
    if meta:
        print("Meta:", json.dumps(meta, ensure_ascii=False))

    tags_df = load_tags_table(root)  # only artefacts
    alias_map = load_alias_map_if_any(root)

    # (2) resolve tag
    tag_id, hints = resolve_tag_ids(tags_df, args.tag, alias_map, suggest=args.suggest)
    if tag_id is None:
        print(f"No tags found for this name '{args.tag}'.", "Possible options:" if hints else "")
        for h in hints:
            print("  ", h)
        print("The exact --tag <id> can be specified.")
        return

    print("Use tag_id =", tag_id)

    # (3) First, try the ready-made KNN.
    if knn_df is not None:
        dfk = (knn_df
