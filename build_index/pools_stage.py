...
def step_post_in_pools_count(cfg: Config) -> None:
    ...
    # Align to mmaps/post_ids.bin
    post_ids_path = cfg.mmaps_dir / "post_ids.bin"
    if not post_ids_path.exists():
        raise FileNotFoundError(
            f"Required primary key array {post_ids_path} not found. "
            "The 'mmaps' stage must be executed before 'pools_post_counts'."
        )
    ...
    if counts:
        pids = np.array(list(counts.keys()), dtype=np.int64)
        cnt_vals = np.array(list(counts.values()), dtype=np.int32)
        idxs = np.searchsorted(post_ids, pids)
        in_bounds = idxs < n_posts
        safe_idxs = np.where(in_bounds, idxs, 0)
        valid = in_bounds & (post_ids[safe_idxs] == pids)
        arr[idxs[valid]] = cnt_vals[valid]
...
def step_pool_tag_co(cfg: Config) -> None:
    ...
    pools = (pl.scan_parquet(f"{long_dir.as_posix()}/**/*.parquet")
             .filter(pl.col("pool_id").is_in(list(elig_ids)))
             .select(["pool_id", "post_id"]))
    post_tags = pl.scan_parquet(f"{pt_dir.as_posix()}/**/*.parquet").select(["post_id", "tag_id"])
    ...
    if rows:
        pl.DataFrame(rows, schema={
            "a_tag_id": pl.Int64, "b_tag_id": pl.Int64,
            "weight": pl.Float32, "pools": pl.Int32
        },
        orient="row"
        ).write_parquet(out, compression="zstd")
        ...
        pl.DataFrame({
            "a_tag_id": pl.Series([], dtype=pl.Int64),
            "b_tag_id": pl.Series([], dtype=pl.Int64),
            "weight": pl.Series([], dtype=pl.Float32),
            "pools": pl.Series([], dtype=pl.Int32),
        }).write_parquet(out, compression="zstd")
